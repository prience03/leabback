...
//        if (TRACE) TraceHelper.endSection();
    ...
    private boolean scrollOther(View focused, int[] deltas) {
        if (focused != null) {
            View parentView = (View) focused.getParent();
            if (parentView != null && null != focused) {
                String tag = (String) parentView.getTag();
                if (tag != null && tag.equals("other")) {
                    int selectedItemOffsetStart = (getFreeHeight() - focused.getHeight());
                    selectedItemOffsetStart /= 2;
                    Rect rect = new Rect();
                    mBaseGridView.offsetDescendantRectToMyCoords(focused, rect);
                    final int dx = 0;
                    int dy = rect.top - selectedItemOffsetStart;
                    deltas[0] = dy;
                    deltas[1] = dx;
                    return true;
                }
    ...
    private int getFreeHeight() {
        return mBaseGridView.getHeight() - mBaseGridView.getPaddingTop() - mBaseGridView.getPaddingBottom();
    }
    ...
    private boolean getAlignedPosition(View view, View childView, int[] deltas) {
        // 用于其它滚动.
        if (scrollOther(childView, deltas)) {
            return true;
        }
